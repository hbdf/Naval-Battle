...
double xx = -3, yy = 8, zz = 3;
void init() {
    player1 = new board(10);
    player2 = new board(10);
...
void change_state() {
    ...
    if(curr_state == PLAYER1_CHOOSE_POS && player1->get_size() == 3) {
        curr_state = PLAYER2_CHOOSE_POS;
    } else if(curr_state == PLAYER2_CHOOSE_POS  && player2->get_size() == 3) {
...
int get_size_ship() {
    if(curr_state == PLAYER1_CHOOSE_POS) {
        return 6 - player1->get_size();
    } else return 6 - player2->get_size();
}
...
//limpa a tela com a cor de fundo
    ...
    glLoadIdentity();
    glColor3f(0, 0, 1);
    gluLookAt(xx, yy, zz, 5, 0, 5, 0, 1, 0);
